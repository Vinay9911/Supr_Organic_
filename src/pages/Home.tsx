    ...
    <div className="pb-20">
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden bg-slate-950">
        <div className="absolute inset-0 opacity-40">
          <img src="https://images.unsplash.com/photo-1615215266453-277e923838dc?q=80&w=2000" className="w-full h-full object-cover" alt="Background" />
      ...
      <section id="labs" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4">
           {/* ... (Existing Labs Content kept same) ... */}
            ...
            {products.filter(p => !p.isLaunchingSoon).map(product => {
              ...
              <div key={product.id} className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100 flex flex-col h-full relative">
                <Link to={`/product/${product.id}`} className="block relative aspect-square overflow-hidden cursor-pointer">
                  ...
                  {product.discountPercentage && <div className="absolute top-4 left-4 bg-emerald-600 text-white text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider">Save {product.discountPercentage}%</div>}
                </Link>
